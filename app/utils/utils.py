...
def load_locales(i18n_dir, supported_languages: Iterable[str] | None = None):
    supported = tuple(
        str(language).strip()
        for language in (supported_languages or [])
        if str(language).strip()
    )
    return _load_locales_cached(str(i18n_dir), supported)


@lru_cache(maxsize=8)
def _load_locales_cached(i18n_dir: str, supported_languages: tuple[str, ...]):
    # WebUI 每次交互都会触发 Streamlit 重新执行脚本，语言文件运行期不会变化，
    # 因此缓存解析结果，避免反复读取和解析所有 i18n JSON 文件。
    _locales = {}
    if supported_languages:
        for lang in supported_languages:
            locale_file = os.path.join(i18n_dir, f"{lang}.json")
