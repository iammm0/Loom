...
class RedisState(BaseState):
    ...
    values are written by Loom and converted back from strings for
...
class SQLiteState(BaseState):
    """Compatibility facade backed by the durable task store."""
    ...
    def update_task(
        ...
        **kwargs,
        ...
        self._store.update_runtime_task(
            task_id,
            state=state,
            progress=progress,
            **kwargs,
    ...
    def get_all_tasks(self, page: int, page_size: int):
        return self._store.list_tasks(page, page_size)

    def patch_task(self, task_id: str, **kwargs) -> bool:
        return self._store.patch_task(task_id, **kwargs)

    def delete_task(self, task_id: str):
        self._store.delete_task(task_id)


# SQLite is the canonical task state for the single-machine application. The
# MemoryState and RedisState classes remain available for compatibility and
# focused tests, but runtime task history must survive WebUI and container
# restarts.
state = SQLiteState()
