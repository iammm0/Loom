...
# 这里显式抬高音频码率，避免作品生成阶段因为默认值过低而引入明显失真。
audio_bitrate = "192k"
fps = 30
...
class InsufficientVisualDurationError(RuntimeError):
    def __init__(self, available: float, required: float):
        self.available = float(available)
        self.required = float(required)
        self.shortfall = max(0.0, self.required - self.available)
        super().__init__(
            f"visual duration {self.available:.3f}s is shorter than required "
            f"{self.required:.3f}s"
        )
...
# Compatibility for tests and callers that imported the former private helper.
_get_required_video_duration = get_required_video_duration


def get_effective_visual_duration(
    video_paths: List[str],
    *,
    max_clip_duration: float,
    clip_speed: float = 1.0,
    clip_durations: List[float] | None = None,
    concat_mode: VideoConcatMode = VideoConcatMode.sequential,
) -> tuple[float, list[float]]:
    """Return the playback capacity that ``combine_videos`` can actually use."""
    normalized_speed = utils.normalize_clip_speed(clip_speed)
    maximum = max(0.1, float(max_clip_duration or 0.1))
    sequential = (
        getattr(concat_mode, "value", concat_mode) == VideoConcatMode.sequential.value
    )
    durations = []
    configured = list(clip_durations or [])
    for index, video_path in enumerate(video_paths):
        source_duration = max(0.0, float(get_media_duration(video_path)))
        playable_duration = source_duration / normalized_speed
        configured_duration = None
        if index < len(configured):
            try:
                configured_duration = max(0.1, float(configured[index]))
            except (TypeError, ValueError):
                configured_duration = None
        if sequential:
            limit = configured_duration or maximum
            playable_duration = min(playable_duration, limit)
        durations.append(round(max(0.0, playable_duration), 3))
    return round(sum(durations), 3), durations
...
def _prioritize_unique_source_clips(
    ...
    random 模式下一个长视频可能被拆成多个候选切片。旧逻辑会在其他素材
    用完后继续使用同一个源视频的剩余切片，仍然会让用户感知为重复镜头。
    现在每个源文件只保留最长的一个候选；素材总时长不足时由上层转入待补
    分镜，不再以重复素材换取任务成功。
    """
    ...
    if concat_mode_value != VideoConcatMode.random.value:
        ...
        for item in subclipped_items:
            if item.source_file_path in seen_sources:
                continue
            seen_sources.add(item.source_file_path)
    ...
    logger.info(
        "enforced unique video materials, "
        f"sources: {len(grouped_items)}, "
        f"selected clips: {len(primary_items)}"
    )
...
def _open_video_clip_quietly(video_path: str, audio: bool = False) -> VideoFileClip:
    ...
    `audio_found: False`。这只是输入素材 metadata，不代表最终作品没有音频，
...
def combine_videos(
    ...
    # max_clip_duration 约束的是作品里的最终播放时长，而不是源视频读取时长。
    ...
    for path_index, video_path in enumerate(video_paths):
        ...
        if clip_durations and path_index < len(clip_durations):
            target_duration = max(0.1, float(clip_durations[path_index]))
        source_duration_for_item = (
            target_duration * normalized_clip_speed
            if target_duration is not None
            else source_clip_duration
        )

        while start_time < clip_duration:
            end_time = min(start_time + source_duration_for_item, clip_duration)
    ...
    for i, subclipped_item in enumerate(subclipped_items):
        ...
        try:
            ...
            target_clip_duration = (
                subclipped_item.duration
                if clip_durations and subclipped_item.duration is not None
                else float(max_clip_duration)
            )
            if clip.duration > target_clip_duration:
                clip = clip.subclipped(0, target_clip_duration)
    ...
    if video_duration < required_video_duration:
        logger.warning(
            ...
            f"({required_video_duration:.2f}s); additional scenes are required."
        )
        delete_files([clip.file_path for clip in processed_clips])
        raise InsufficientVisualDurationError(video_duration, required_video_duration)
...
def generate_video(
    ...
    # Windows PermissionError: [WinError 32] The process cannot access the file
    # because it is being used by another process (MoviePy temp audio).
    ...
    with ExitStack() as clip_stack:
        ...
        if bgm_file:
            try:
                ...
                # 服务内解析的随机/自定义音乐可能比作品短，需要循环铺满；任务层
