...
def create_edge_tts_communicate(
    ...
    # edge_tts 的语音合成使用 WebSocket。aiohttp 不会自动读取 macOS
    # “系统设置”里的代理，只会读取环境变量或显式 proxy 参数，因此复用
    # 项目 [proxy] 配置，避免在受限办公网络中绕过本机代理后持续超时。
    if "proxy" in communicate_signature.parameters:
        edge_tts_proxy = config.proxy.get("https") or config.proxy.get("http")
        if isinstance(edge_tts_proxy, str) and edge_tts_proxy.strip():
            communicate_kwargs["proxy"] = edge_tts_proxy.strip()
...
def chatterbox_tts(
    ...
    payload = {
        ...
        "voice": voice,
        "response_format": "mp3",
        # OpenAI speech API accepts speed 0.25-4.0; Loom's rate is a
