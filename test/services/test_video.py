...
class TestVideoService(unittest.TestCase):
    ...
    def test_effective_visual_duration_matches_sequential_composition_limits(self):
        with patch.object(
            vd, "get_media_duration", side_effect=[9.5, 3.0, 12.0]
        ):
            total, durations = vd.get_effective_visual_duration(
        ...
        self.assertEqual(durations, [6.0, 3.0, 6.0])
        self.assertEqual(total, 15.0)

    def test_effective_visual_duration_accounts_for_speed_and_target_duration(self):
        with patch.object(vd, "get_media_duration", return_value=10.0):
            total, durations = vd.get_effective_visual_duration(
                ["one.mp4"],
                max_clip_duration=6,
                clip_speed=2.0,
    ...
    def test_preprocess_video(self):
        if not os.path.exists(self.test_img_path):
            self.skipTest(f"test image not found: {self.test_img_path}")
    ...
    def test_combine_videos_handles_none_transition_mode(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(vd, "AudioFileClip", return_value=_FakeAudioClip()):
                # Empty visual input must surface an explicit shortage instead of
                # silently returning a nonexistent output or looping content.
                with self.assertRaises(vd.InsufficientVisualDurationError):
                    vd.combine_videos(
    ...
    def test_combine_videos_keeps_small_duration_safety_margin(self):
        ...
        在 10.0s == 10.0s 时立即停止，作品末尾就可能出现音频还在播放但
        视频素材已经结束的边界问题。
        """
    ...
    def test_prioritize_unique_source_clips_uses_each_source_only_once(self):
        """
        随机模式下，一个长素材会被拆成多个片段，但一条成片只能选择其中
        一个片段。画面不足应转为待补素材，不能继续复用同一源视频。
        """
        clips = [
        ...
        self.assertCountEqual(
            [clip.source_file_path for clip in ordered_clips],
            ["a.mp4", "b.mp4", "c.mp4"],
        )
    ...
    def test_prioritize_unique_source_clips_deduplicates_sequential_sources(self):
        first = vd.SubClippedVideoClip(
            "a.mp4", 0, 4, source_file_path="a.mp4"
        )
        repeated = vd.SubClippedVideoClip(
            "a.mp4", 0, 4, source_file_path="a.mp4"
        )
        second = vd.SubClippedVideoClip(
            "b.mp4", 0, 4, source_file_path="b.mp4"
        )

        ordered_clips = vd._prioritize_unique_source_clips(
            subclipped_items=[first, repeated, second],
            concat_mode=vd.VideoConcatMode.sequential,
        )

        self.assertEqual(ordered_clips, [first, second])
