...
class TestCli(unittest.TestCase):
    ...
    def test_build_video_params_uses_ai_generated_material_strategy(self):
        args = cli.parse_args(
            [
                "--video-subject",
                "测试主题",
        ...
        self.assertEqual(params.video_sources, ["manual_upload"])
    ...
    def test_run_cli_returns_error_when_task_fails(self):
        with patch("app.services.task.start", return_value=None), patch(
            ...
            code = cli.run_cli(["--video-subject", "失败场景", "--stop-at", "script"])
    ...
    def test_non_ai_video_source_is_rejected(self):
        with self.assertRaises(SystemExit) as cm:
    ...
    def test_custom_audio_file_is_rejected(self):
        with self.assertRaises(SystemExit) as cm:
            cli.parse_args(
                [
                    "--video-subject",
    ...
    def test_local_source_is_rejected(self):
        with self.assertRaises(SystemExit) as cm:
            cli.parse_args(["--video-subject", "test", "--video-source", "local"])
        self.assertEqual(cm.exception.code, 2)

    def test_video_materials_are_rejected(self):
        with self.assertRaises(SystemExit) as cm:
            cli.parse_args([
                "--video-subject", "test",
    ...
    def test_prepare_cli_files_clears_legacy_custom_audio(self):
        from app.models.schema import VideoParams

        params = VideoParams(
            video_subject="test",
            custom_audio_file="/tmp/legacy-voiceover.mp3",
        )
