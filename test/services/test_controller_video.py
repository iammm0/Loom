...
class TestVideoControllerTasks(unittest.TestCase):
    ...
    def test_upload_scene_material_validates_and_attaches_video(self):
        store = MagicMock()
        store.get_task.return_value = {
            "task_id": "task-123",
            "status": const.TASK_STATUS_AWAITING_MATERIAL,
        }
        store.attach_scene_material.return_value = (
            {"task_id": "task-123", "uploaded_scene_paths": {"1": "saved.mp4"}},
            "",
        )
        upload = SimpleNamespace(filename="scene.mp4", file=BytesIO(b"video"))

        with tempfile.TemporaryDirectory() as temp_dir:
            with (
                patch.object(
                    video_controller.task_store,
                    "get_task_store",
                    return_value=store,
                ),
                ...
                response = video_controller.upload_task_scene_material(
                    self._request(), upload, task_id="task-123", scene_index=1
                )
                saved_path = store.attach_scene_material.call_args.args[2]
                self.assertTrue(os.path.isfile(saved_path))
                self.assertEqual(Path(saved_path).read_bytes(), b"video")

        self.assertEqual(response["status"], 200)
        store.attach_scene_material.assert_called_once_with(
            "task-123", 1, saved_path, original_name="scene.mp4"
        )

    def test_confirm_scene_materials_requeues_task(self):
        store = MagicMock()
        store.confirm_scene_materials.return_value = {
            "task_id": "task-123",
        ...
        with (
            ...
            patch.object(
                video_controller.task_store,
                "ensure_task_workers_started",
            ) as start,
        ):
            response = video_controller.confirm_task_scene_materials(
                self._request(), task_id="task-123"
            )

        self.assertEqual(response["status"], 200)
        store.confirm_scene_materials.assert_called_once_with("task-123")
    ...
    def test_create_task_allows_oversized_custom_script_before_queue(self):
        body = video_controller.TaskVideoRequest(
            video_subject="Long",
            video_script="这是一段明显过长的旁白。" * 120,
            ai_director_enabled=False,
        )
        with (
            patch.object(video_controller.utils, "get_uuid", return_value="long-task"),
            patch.object(video_controller.sm.state, "update_task") as update_task,
            patch.object(video_controller.task_manager, "add_task") as add_task,
        ):
            response = video_controller.create_task(
        ...
        self.assertEqual(response["status"], 200)
        self.assertEqual(response["data"]["task_id"], "long-task")
        update_task.assert_called_once_with("long-task")
        add_task.assert_called_once_with(
            video_controller.tm.start,
            task_id="long-task",
            params=body,
    ...
    def test_approve_supplemental_scenes_requeues_task(self):
        store = MagicMock()
        store.approve_supplemental_scenes.return_value = {
            "task_id": "task-123",
            "status": const.TASK_STATUS_QUEUED,
        }
        with (
            patch.object(video_controller.task_store, "get_task_store", return_value=store),
            patch.object(video_controller.task_store, "ensure_task_workers_started") as start,
        ):
            response = video_controller.approve_supplemental_scenes(
                self._request(),
                SupplementalSceneApprovalRequest(prompts={"2": "edited"}),
                task_id="task-123",
            )

        self.assertEqual(response["status"], 200)
        store.approve_supplemental_scenes.assert_called_once_with(
            "task-123", {"2": "edited"}
        )
        start.assert_called_once_with()
    ...
    def test_task_query_preserves_structured_failure_details(self):
        ...
        data = response["data"]
        self.assertEqual(data["task_id"], "failed-task")
        self.assertEqual(data["state"], const.TASK_STATE_FAILED)
        self.assertEqual(data["progress"], 30)
        self.assertEqual(data["failed_stage"], "audio")
        self.assertEqual(data["error"], "TTS request timed out")
