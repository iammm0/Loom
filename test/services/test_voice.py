...
from app.models.schema import DEFAULT_TTS_VOICE_NAME
...
class TestVoiceService(unittest.TestCase):
    ...
    def test_default_voice_dispatches_to_mimo_tts(self):
        expected_sub_maker = object()

        with patch.object(
            vs, "mimo_tts", return_value=expected_sub_maker
        ) as mimo_tts:
            result = vs.tts(
                text="默认小米配音",
                voice_name=DEFAULT_TTS_VOICE_NAME,
                voice_rate=1.0,
                voice_file="/tmp/default-mimo.mp3",
            )

        self.assertIs(result, expected_sub_maker)
        mimo_tts.assert_called_once_with(
            "默认小米配音",
            "mimo_default",
            1.0,
            "/tmp/default-mimo.mp3",
            1.0,
        )
    ...
    def test_create_edge_tts_communicate_uses_configured_https_proxy(self):
        captured = {}

        class _ProxyAwareCommunicate:
            def __init__(
                self,
                text,
                voice,
        ...
        self.assertIsInstance(communicate, _ProxyAwareCommunicate)
