...
def test_director_plan_does_not_reinterpret_paragraphs_as_scenes():
    params = VideoParams(
    ...
    with patch.object(tm.task_store, "get_task_store", return_value=store):
        result = tm.prepare_director_plan("task-id", params, "已经生成的文案。")

    assert params.paragraph_number == 2
    assert params.video_clip_duration == 6
...
class TestTaskService(unittest.TestCase):
    def setUp(self):
        ...
        self._director_patcher = patch.object(
            tm,
            "prepare_director_plan",
            return_value={"source": "test"},
        )
        self._director_patcher.start()

    def tearDown(self):
        self._director_patcher.stop()
        with tm._cross_post_registry_lock:
            tm._cross_post_futures.clear()
    ...
    def test_start_regenerates_corrupt_audio_checkpoint(self):
        """断点音频不可读时应重新 TTS，不能让恢复任务直接失败。"""
        state = MemoryState()
        state.update_task(
            "corrupt-audio-checkpoint",
            state=tm.const.TASK_STATE_PROCESSING,
            progress=20,
            status=tm.const.TASK_STATUS_PROCESSING,
            stage="audio",
            script="existing script",
            audio_file="corrupt-audio.mp3",
        )
        params = VideoParams(video_subject="Coffee")

        with (
            patch.object(tm.sm, "state", state),
            patch.object(tm.path, "isfile", return_value=True),
            patch.object(
                tm.video,
                "get_audio_duration",
                side_effect=OSError("invalid media"),
            ),
            ...
            patch.object(
                tm,
                "generate_audio",
                return_value=("new-audio.mp3", 12, object()),
            ) as generate_audio,
        ):
            result = tm.start(
                "corrupt-audio-checkpoint",
        ...
        self.assertEqual(result["audio_file"], "new-audio.mp3")
    ...
    def test_generate_final_videos_stops_before_next_sonilo_call_after_cancel(self):
        """取消请求到达后不能继续为下一条作品调用付费配乐。"""
        params = VideoParams(
            video_subject="test",
            video_count=2,
            bgm_type="sonilo",
        ...
        def generate_video(**kwargs):
            state.patch_task(
                ...
                status=tm.const.TASK_STATUS_CANCELLATION_REQUESTED,
            )
            return True
        ...
        with (
            ...
            patch.object(tm.video, "generate_video", side_effect=generate_video),
        ):
            with self.assertRaises(tm._TaskCancellationCheckpoint):
                tm.generate_final_videos(
                    task_id="cancel-sonilo",
                    params=params,
                    downloaded_videos=["material.mp4"],
                    audio_file="audio.mp3",
                    subtitle_path="",
        ...
        self.assertEqual(generate_bgm.call_count, 1)
        self.assertEqual(combine_videos.call_count, 1)
    ...
    def test_start_does_not_require_seedance_key_for_manual_scene_materials(self):
        """提示词加人工上传流程不应再把 Seedance Key 作为预检条件。"""
        ...
        with (
            ...
            result = tm.start("missing-seedance-key", params)

        generate_script.assert_called_once_with("missing-seedance-key", params)
        generate_audio.assert_not_called()
        get_materials.assert_not_called()
        failed_task = state.get_task("missing-seedance-key")
        self.assertEqual(result, failed_task)
        self.assertEqual(failed_task["state"], tm.const.TASK_STATE_FAILED)
    ...
    def test_start_generates_terms_for_online_material_provider(self):
        """在线素材策略需要生成按分镜排序的检索词。"""
        params = VideoParams(
            video_subject="startup story",
            video_script="A short startup story.",
            material_strategy="single",
            video_source="pexels",
        )
        state = MemoryState()
        ...
        with (
            ...
            patch.object(
                tm, "generate_terms", return_value=["startup office"]
            ) as generate_terms,
            patch.object(tm, "get_video_materials") as get_video_materials,
            patch.object(tm, "save_script_data"),
            patch.object(tm.sm, "state", state),
        ):
            result = tm.start("legacy-material-provider", params, stop_at="terms")

        generate_terms.assert_called_once()
        get_video_materials.assert_not_called()
        self.assertEqual(
            result, {"script": "generated script", "terms": ["startup office"]}
        )
        self.assertEqual(params.video_source, "pexels")
        self.assertEqual(params.material_strategy, "single")
        self.assertIsNone(params.video_materials)

    def test_generate_audio_ignores_custom_file_and_uses_system_tts(self):
        task_id = "test-custom-audio-ignored"
        params = VideoParams(
            video_subject="custom audio",
            video_script="",
            custom_audio_file="/tmp/custom-audio.mp3",
        ...
        self.assertEqual(audio_file, os.path.join(utils.task_dir(task_id), "audio.mp3"))
        self.assertEqual(audio_duration, 7)
        self.assertIs(result_sub_maker, sub_maker)
        self.assertIsNone(params.custom_audio_file)
        tts.assert_called_once()

    def test_generate_audio_prefers_exact_media_duration(self):
        params = VideoParams(video_subject="Coffee")
        sub_maker = object()
        with (
            patch.object(tm.voice, "tts", return_value=sub_maker),
            patch.object(tm.path, "isfile", return_value=True),
            patch.object(tm.video, "get_audio_duration", return_value=7.25),
            patch.object(tm.voice, "get_audio_duration") as subtitle_duration,
        ):
            _, audio_duration, _ = tm.generate_audio("task-id", params, "旁白")

        self.assertEqual(audio_duration, 7.25)
        subtitle_duration.assert_not_called()

    def test_generate_audio_uses_tts_even_when_legacy_custom_file_is_missing(self):
        task_id = "test-custom-audio-missing-ignored"
        params = VideoParams(
            video_subject="custom audio",
            video_script="",
            custom_audio_file="/tmp/does-not-exist.mp3",
            voice_name="test-voice",
        )
        sub_maker = object()
    ...
    def test_start_completes_video_without_cross_posting(self):
        ...
        self.assertEqual(result["original_videos"], ["final.mp4"])
    ...
    def test_start_persists_model_token_usage(self):
        """完整视频任务完成后必须在返回值和任务状态中保存模型 token 消耗。"""
        params = VideoParams(video_subject="Coffee")
        state = MemoryState()
        def generate_script_with_usage(*args, **kwargs):
            tm.llm._record_model_token_usage(
                llm_provider="openai",
                model_name="gpt-test",
                prompt_tokens=10,
                completion_tokens=5,
        ...
        with (
            patch.object(tm, "generate_script", side_effect=generate_script_with_usage),
            patch.object(
                tm,
                "generate_audio",
                return_value=("audio.mp3", 5, object()),
            ...
            result = tm.start("token-usage-task", params)

        persisted = state.get_task("token-usage-task")
        self.assertEqual(
            result["model_token_usage"]["models"][0],
            {
                "provider": "openai",
                "model": "gpt-test",
                "prompt_tokens": 10,
        ...
        self.assertEqual(persisted["model_token_usage"], result["model_token_usage"])

    def test_get_video_materials_records_ai_provider_error(self):
        """AI 视频素材失败时，任务错误应保留 provider 返回的具体原因。"""
        task_id = "failed-ai-materials"
        ...
        with (
            ...
            patch.object(
                ...
                "prepare_ai_generated_materials",
                return_value={
                    "paths": [],
                    "missing": [],
                    "errors": [{"scene_index": 0, "error": provider_error}],
                    "status": tm.const.TASK_STATUS_FAILED,
                },
            ),
        ):
        ...
        self.assertEqual(task["failed_stage"], "materials")
        self.assertIn("scene 0", task["error"])
        self.assertIn("parameter ratio", task["error"])

    def test_get_video_materials_never_uses_uploads_to_hide_seedance_failure(self):
        """必需分镜失败时不能只拿上传图继续合成一个严重缺时长的作品。"""
        task_id = "failed-ai-materials-with-upload"
        params = VideoParams(
            video_subject="请展示上传截图",
            material_strategy="ai_generated",
            video_materials=[
                MaterialInfo(provider="visual_reference", url="screenshot.png")
            ],
        )
        state = MemoryState()
        state.update_task(
        ...
        with (
            ...
            patch.object(
                tm,
                "_prepare_uploaded_visual_reference_paths",
                return_value=["uploaded-reference.mp4"],
            ),
            patch.object(
                tm.material_pipeline,
                "prepare_ai_generated_materials",
                return_value={
                    "paths": [],
                    "missing": [{"scene_index": 3}],
                    "errors": [
                        {"scene_index": 3, "error": "provider generation failed"}
                    ],
                    "status": tm.const.TASK_STATUS_FAILED,
                },
            ),
            ...
            result = tm.get_video_materials(
                ...
                video_terms=[],
                audio_duration=60,
                video_script="请展示上传截图",
            )

        self.assertIsNone(result)
        task = state.get_task(task_id)
        self.assertEqual(task["failed_stage"], "materials")
        self.assertIn("provider generation failed", task["error"])

    def test_get_video_materials_uses_restored_local_material_strategy(self):
        """本地素材策略应走素材库与在线素材的有序匹配流程。"""
        params = VideoParams(
            video_subject="Coffee",
            video_source="local",
            video_sources=["local"],
            material_strategy="single",
            video_materials=[MaterialInfo(provider="local", url="local.mp4")],
        )

        with (
            patch.object(
                tm.material_pipeline,
                "prepare_ordered_materials",
                return_value={
                    "paths": ["local-scene.mp4"],
                    "missing": [],
                    "errors": [],
                    "status": tm.const.TASK_STATUS_PROCESSING,
                },
            ) as prepare_ordered,
            patch.object(tm.video, "preprocess_video") as preprocess_video,
        ):
            result = tm.get_video_materials(
                "legacy-local-materials",
                params,
        ...
        self.assertEqual(result, ["local-scene.mp4"])
        preprocess_video.assert_not_called()
        prepare_ordered.assert_called_once()
        self.assertEqual(params.video_source, "local")
        self.assertEqual(params.video_sources, ["local"])
        self.assertEqual(params.material_strategy, "single")

    def test_get_video_materials_keeps_uploaded_visuals_until_requested(self):
        """上传截图或 Logo 后，未在诉求/文案中要求展示时不应强行剪入。"""
        params = VideoParams(
            video_subject="Coffee",
            material_strategy="ai_generated",
            video_materials=[
        ...
        self.assertEqual(result, ["ai-scene.mp4"])
        preprocess_video.assert_not_called()
        self.assertEqual(params.video_source, "manual_upload")
        self.assertEqual(params.material_strategy, "ai_generated")
        self.assertEqual(len(params.video_materials), 1)

    def test_get_video_materials_inserts_uploaded_visuals_when_requested(self):
        """诉求或文案明确提到上传截图/Logo 时，应把上传图插入素材序列。"""
        params = VideoParams(
            video_subject="请展示上传截图，并结合产品 Logo 做介绍",
            material_strategy="ai_generated",
            video_clip_duration=3,
            video_materials=[
                MaterialInfo(provider="visual_reference", url="screenshot.png"),
                MaterialInfo(provider="visual_reference", url="logo.png"),
            ],
        )
        prepared_screenshot = MaterialInfo(
            provider="visual_reference",
            url="prepared-screenshot.mp4",
        )
        prepared_logo = MaterialInfo(
            provider="visual_reference",
            url="prepared-logo.mp4",
        )
        store = MagicMock()
        store.get_task.side_effect = [
            {},
            {"selected_material_durations": [3.2, 4.1]},
        ]
        ...
        with (
            patch.object(
                ...
                "prepare_ai_generated_materials",
                return_value={
                    "paths": ["ai-scene-1.mp4", "ai-scene-2.mp4"],
                    "missing": [],
                    "errors": [],
                    "status": tm.const.TASK_STATUS_PROCESSING,
                },
            ...
            patch.object(
                tm.video,
                "preprocess_video",
                return_value=[prepared_screenshot, prepared_logo],
            ) as preprocess_video,
            patch.object(
                tm.video,
                "get_media_duration",
                side_effect=lambda path: {
                    "prepared-screenshot.mp4": 2.5,
                    "prepared-logo.mp4": 2.8,
                }[path],
            ),
            patch.object(tm.task_store, "get_task_store", return_value=store),
        ):
            result = tm.get_video_materials(
                ...
                video_terms=[],
                audio_duration=8,
                video_script="第一段介绍系统界面。第二段展示logo。",
            )

        self.assertEqual(
            result,
            [
                "prepared-screenshot.mp4",
                "ai-scene-1.mp4",
                "prepared-logo.mp4",
                "ai-scene-2.mp4",
            ],
        ...
        self.assertEqual(preprocess_video.call_args.kwargs["clip_duration"], 3)
        store.patch_task.assert_called_once_with(
            "requested-visual-reference",
            selected_materials=result,
            selected_material_durations=[2.5, 3.2, 2.8, 4.1],
        )
        self.assertEqual(params.video_source, "manual_upload")
        self.assertEqual(params.material_strategy, "ai_generated")

    def test_start_fails_when_scenes_missing_and_cannot_auto_fill(self):
        params = VideoParams(
            video_subject="Coffee",
            material_strategy="ai_generated",
        )
        state = MemoryState()
        final_videos = MagicMock()

        def wait_for_scene_uploads(task_id, *_args, **_kwargs):
            scenes = [
                {
                    "scene_index": 0,
                    "text": "第一段",
            ...
            state.update_task(
                ...
                state=tm.const.TASK_STATE_PROCESSING,
                progress=45,
                status=tm.const.TASK_STATUS_AWAITING_MATERIAL,
        ...
        with (
            ...
            patch.object(
                tm,
                "generate_audio",
                return_value=("audio.mp3", 6, object()),
            ),
            patch.object(tm, "generate_subtitle", return_value="subtitle.srt"),
            patch.object(
                tm,
            ...
            patch.object(tm, "generate_final_videos", final_videos),
            patch.object(tm.sm, "state", state),
        ):
            result = tm.start("awaiting-scene-uploads", params)

        failed_task = state.get_task("awaiting-scene-uploads")
        self.assertEqual(failed_task["state"], tm.const.TASK_STATE_FAILED)
        self.assertEqual(failed_task["failed_stage"], "materials")
        self.assertEqual(result, failed_task)
        final_videos.assert_not_called()

    def test_start_auto_fills_visual_shortfall_instead_of_pausing(self):
        params = VideoParams(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for index in range(4):
                material_path = Path(temp_dir) / f"scene-{index}.mp4"
                material_path.write_bytes(b"video")
                materials.append(str(material_path))
            extra = Path(temp_dir) / "scene-4.mp4"
            ...
            with (
                ...
                patch.object(
                    tm,
                    "generate_audio",
                    return_value=("audio.mp3", 37.66, object()),
                ),
                patch.object(tm, "generate_subtitle", return_value="subtitle.srt"),
                patch.object(
                    tm,
                    "get_video_materials",
                    side_effect=[materials, materials + [str(extra)]],
                ),
                patch.object(
                    tm.video,
                    "get_effective_visual_duration",
                    return_value=(24.0, [6.0, 6.0, 6.0, 6.0]),
                ),
                patch(
                    "app.agent.graph.material_pipeline.build_visual_duration_shortfall_scenes",
                    return_value={
                        "new_supplemental_scenes": [
                            {"scene_index": 4, "target_duration": 4.5, "material_prompt": "broll"}
                        ]
                    },
                ),
                patch(
                    "app.tools.production._auto_generate_missing",
                    return_value=[str(extra)],
                ) as auto_fill,
                patch.object(tm, "generate_final_videos", final_videos),
                patch.object(tm.sm, "state", state),
            ):
                result = tm.start("short-visuals", params)

        auto_fill.assert_called_once()
        self.assertEqual(result["status"], tm.const.TASK_STATUS_COMPLETED)
        final_videos.assert_called_once()
    ...
    def test_start_allows_narration_longer_than_60_seconds(self):
        params = VideoParams(video_subject="Coffee")
        state = MemoryState()
        materials = MagicMock()
        ...
        with (
            ...
            patch.object(
                tm,
                "generate_audio",
                return_value=("audio.mp3", 61, object()),
            ),
            patch.object(
                tm,
                "get_video_materials",
            ...
            materials.return_value = ["material.mp4"]
            result = tm.start("overlong-narration", params, stop_at="materials")

        completed_task = state.get_task("overlong-narration")
        self.assertEqual(result, {"materials": ["material.mp4"]})
        self.assertEqual(completed_task["status"], tm.const.TASK_STATUS_COMPLETED)
        materials.assert_called_once()
        self.assertEqual(materials.call_args.args[3], 61)
    ...
    def test_start_generates_youtube_metadata_for_each_cross_post(self):
        ...
        for index, call in enumerate(cross_post.call_args_list, start=1):
            self.assertEqual(call.kwargs["video_path"], f"final-{index}.mp4")
