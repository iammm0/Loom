...
class TestScriptPromptOptions(unittest.TestCase):
    def test_auto_duration_does_not_set_a_time_target(self):
        ...
        self.assertIn("自然确定文案长度", prompt)
        self.assertNotIn("作品时长目标", prompt)
        self.assertNotIn("秒", prompt)

    def test_script_prompt_includes_fuzzy_duration_and_auto_paragraphs(self):
        prompt = llm.build_script_prompt(
            video_subject="城市咖啡",
            paragraph_number=3,
        ...
        self.assertIn("约 30～60 秒", prompt)
        self.assertIn("围绕 45 秒", prompt)
        self.assertIn("由你根据内容节奏自然确定", prompt)

    def test_legacy_long_duration_keeps_requested_range(self):
        prompt = llm.build_script_prompt(
            video_subject="城市咖啡",
            target_duration="180-300",
        )

        self.assertIn("约 180～300 秒", prompt)

    def test_generate_script_keeps_result_over_previous_limit(self):
        long_script = "这是一段明显过长的旁白。" * 140
        with patch.object(
            llm,
            "_generate_response",
            return_value=long_script,
        ) as generate:
            result = llm.generate_script(video_subject="城市咖啡")

        self.assertEqual(result, long_script)
        self.assertEqual(generate.call_count, 1)
        self.assertGreater(llm.estimate_narration_duration(result), 60)

    def test_script_prompt_keeps_manual_paragraph_override(self):
        prompt = llm.build_script_prompt(
            video_subject="城市咖啡",
            paragraph_number=3,
            target_duration="auto",
        ...
        self.assertIn("- 文案段落数：3", prompt)

    def test_generate_director_plan_parses_and_clamps_json(self):
        response = json.dumps(
            {
                "paragraph_number": 4,
                "voice_rate": 1.1,
                "video_clip_duration": 8,
                "video_clip_speed": 1.05,
        ...
        self.assertNotIn("paragraph_number", plan)
        self.assertEqual(plan["video_transition_mode"], "FadeIn")
        self.assertEqual(plan["voice_rate"], 1.1)

    def test_generate_director_plan_falls_back_without_enabling_services(self):
        ...
        self.assertEqual(plan["video_clip_duration"], 5)
        self.assertEqual(plan["sonilo_bgm_prompt"], "")
    ...
    def test_build_script_prompt_appends_advanced_requirements(self):
        ...
        self.assertIn("# 角色：短视频文案生成器", prompt)
        self.assertIn("- 视频主题：咖啡", prompt)
        self.assertIn("- 文案段落数：3", prompt)
        self.assertIn("- 文案语言：zh-CN", prompt)
        self.assertIn("# 附加要求", prompt)
    ...
    def test_custom_system_prompt_keeps_runtime_context(self):
        ...
        self.assertNotIn("# 角色：短视频文案生成器", prompt)
        self.assertIn("Only write cinematic narration.", prompt)
        self.assertIn("- 视频主题：露营", prompt)
        self.assertIn("- 文案段落数：2", prompt)
        self.assertIn("- 文案语言：en", prompt)
    ...
    def test_generate_script_sends_custom_prompt_to_llm(self):
        ...
        self.assertIn("- 文案段落数：2", captured["prompt"])
    ...
    def test_build_video_scene_prompts_prompt_contains_scene_contract(self):
        prompt = llm.build_video_scene_prompts_prompt(
            video_subject="城市咖啡",
        ...
        self.assertIn("AI Video Scene Prompt Director", prompt)
        self.assertIn('"scene_index":0', prompt)
        self.assertIn('"video_prompt"', prompt)
        self.assertIn("Do not add, remove, merge, or reorder scenes", prompt)

    def test_generate_video_scene_prompts_parses_fenced_json(self):
        with patch.object(
            llm,
            "_generate_response",
            return_value=(
                '```json\n{"scenes":['
                '{"scene_index":0,"video_prompt":"cinematic coffee shop","negative_prompt":"text"},'
                '{"scene_index":1,"video_prompt":"morning street","negative_prompt":"logo"}'
                "]}\n```"
            ),
        ):
            result = llm.generate_video_scene_prompts(
                video_subject="城市咖啡",
                video_script="第一段。第二段。",
                scenes=[
                    {"scene_index": 0, "text": "第一段。", "target_duration": 3},
        ...
        self.assertEqual(
            [scene["video_prompt"] for scene in result],
            ["cinematic coffee shop", "morning street"],
        )
        self.assertEqual(result[0]["negative_prompt"], "text")

    def test_generate_video_scene_prompts_falls_back_on_provider_error(self):
        with patch.object(
            llm,
            "_generate_response",
            return_value="Error: invalid API key",
            ...
            result = llm.generate_video_scene_prompts(
                video_subject="城市咖啡",
                video_script="第一段。",
                scenes=[{"scene_index": 0, "text": "第一段。", "target_duration": 3}],
                aspect_ratio="9:16",
            )

        self.assertEqual(result[0]["scene_index"], 0)
        self.assertIn("第一段", result[0]["video_prompt"])
        self.assertIn("水印", result[0]["negative_prompt"])

    def test_generate_video_scene_plan_uses_unit_ranges_for_duration(self):
        units = [
            {"unit_index": 0, "text": "第一", "start": 0.0, "end": 1.5},
            {"unit_index": 1, "text": "段。", "start": 1.5, "end": 3.0},
            {"unit_index": 2, "text": "第二段。", "start": 3.0, "end": 6.0},
        ]
        response = {
            "scenes": [
                {
                    "unit_start": 0,
                    "unit_end": 1,
                    "visual_subject": "coffee beans",
                    "action": "beans falling",
                    "environment": "roastery",
                    "shot_type": "close-up",
                    "camera_motion": "slow push-in",
                    "lighting": "warm light",
                    "video_prompt": "coffee beans falling in a warm roastery",
                    "negative_prompt": "text",
                },
                {
                    "unit_start": 2,
                    "unit_end": 2,
                    "visual_subject": "barista",
                    "action": "serving coffee",
                    "environment": "street cafe",
                    "shot_type": "wide shot",
                    "camera_motion": "tracking shot",
                    "lighting": "morning light",
                    "video_prompt": "barista serving coffee in a street cafe",
                    "negative_prompt": "logo",
                },
            ]
        }
        with patch.object(llm, "_generate_response", return_value=json.dumps(response)):
            scenes = llm.generate_video_scene_plan(
                video_subject="咖啡",
                video_script="第一段。第二段。",
        ...
        self.assertEqual([scene["target_duration"] for scene in scenes], [3.0, 3.0])
        self.assertEqual("".join(scene["text"] for scene in scenes), "第一段。第二段。")

    def test_generate_video_scene_plan_fallback_covers_all_units(self):
        units = [
            {"unit_index": index, "text": str(index), "start": index, "end": index + 1}
            for index in range(8)
        ]
        with patch.object(llm, "_generate_response", return_value="not-json"):
            scenes = llm.generate_video_scene_plan(
                video_subject="测试",
                video_script="01234567",
                narration_units=units,
                aspect_ratio="9:16",
                maximum_duration=3,
            )
        ...
        covered = [
            unit_index
            for scene in scenes
            for unit_index in range(scene["unit_start"], scene["unit_end"] + 1)
        ]
        self.assertEqual(covered, list(range(8)))
        self.assertTrue(all(scene["target_duration"] <= 3 for scene in scenes))
        self.assertNotEqual(scenes[0]["shot_type"], scenes[1]["shot_type"])

    def test_generate_video_scene_plan_allows_timeline_over_60_seconds(self):
        units = [
            {
                "unit_index": index,
                "text": f"第{index}段",
        ...
        self.assertEqual(sum(scene["target_duration"] for scene in scenes), 75)
        self.assertEqual(
            [
                unit_index
                for scene in scenes
...
class TestLiteLLMProvider(unittest.TestCase):
    ...
    def test_provider_tip_templates_accept_registry_defaults(self):
        ...
        if not i18n_dir.is_dir():
            self.skipTest("Streamlit i18n 已移除")
    ...
    def test_primary_provider_tips_use_consistent_structure(self):
        ...
        i18n_dir = Path(__file__).parent.parent.parent / "webui" / "i18n"
        if not (i18n_dir / "zh.json").is_file():
            self.skipTest("Streamlit i18n 已移除")
    ...
    def test_generate_response_records_api_token_usage(self):
        """任务上下文中应按 provider/model 汇总 SDK 返回的 token usage。"""
        config.app.update(
            {
                "llm_provider": "openai",
        ...
        with (
            llm.token_usage_context() as token_usage,
            patch.object(llm, "OpenAI", return_value=fake_client),
            patch.object(llm, "ChatCompletion", types.SimpleNamespace),
        ):
            result = llm._generate_response("Say hello")

        summary = llm.get_collected_token_usage(token_usage)
        ...
        self.assertEqual(summary["total"]["total_tokens"], 18)

    def test_generate_response_estimates_token_usage_when_api_usage_missing(self):
        """Provider 不返回 usage 时，任务结果仍应给出明确标记的估算值。"""
        config.app.update(
            {
                "llm_provider": "openai",
                "openai_api_key": "openai-test-key",
                "openai_base_url": "",
                "openai_model_name": "gpt-estimated",
            }
        )
        ...
        self.assertTrue(summary["models"][0]["estimated"])
        self.assertGreater(summary["models"][0]["total_tokens"], 0)
    ...
    def test_openai_provider_forwards_configured_default_headers(self):
        config.app["llm_provider"] = "openai"
        config.app["openai_api_key"] = "proxy-key"
        config.app["openai_base_url"] = "https://proxy.example.com/v1"
        config.app["openai_model_name"] = "proxy-model"
        config.app["openai_default_headers"] = {
        ...
        fake_response = types.SimpleNamespace(
            choices=[
                types.SimpleNamespace(
                    message=types.SimpleNamespace(content="proxy\nworks")
                )
            ]
        )
        fake_client = types.SimpleNamespace(
        ...
        self.assertEqual(result, "proxyworks")

    def test_openai_provider_rejects_non_table_default_headers(self):
        config.app["llm_provider"] = "openai"
        config.app["openai_api_key"] = "proxy-key"
        config.app["openai_base_url"] = "https://proxy.example.com/v1"
        config.app["openai_model_name"] = "proxy-model"
        config.app["openai_default_headers"] = "invalid"

        result = llm._generate_response("Say hello")

        self.assertIn("Error:", result)
        self.assertIn("default_headers must be a TOML table", result)
...
@unittest.skipUnless(
    RUN_INTEGRATION_TESTS and os.environ.get("FOUNDRY_MODEL"),
    "live LiteLLM credentials are not configured",
)
class TestLiteLLMLiveIntegration(unittest.TestCase):
    def setUp(self):
        self.original_app_config = dict(config.app)
        config.app["llm_provider"] = "litellm"
        config.app["litellm_model_name"] = os.environ["FOUNDRY_MODEL"]
        os.environ["AZURE_AI_API_KEY"] = os.environ.get("AZURE_AI_API_KEY", "")
        os.environ["AZURE_AI_API_BASE"] = os.environ.get("AZURE_AI_API_BASE", "")
