...
class TestSubtitleBackgroundSettings(unittest.TestCase):
    def test_task_requests_default_to_mimo_tts(self):
        video_params = VideoParams(video_subject="default voice")
        subtitle_request = SubtitleRequest(video_script="default voice")
        audio_request = AudioRequest(video_script="default voice")

        self.assertEqual(video_params.voice_name, DEFAULT_TTS_VOICE_NAME)
        self.assertEqual(subtitle_request.voice_name, DEFAULT_TTS_VOICE_NAME)
        self.assertEqual(audio_request.voice_name, DEFAULT_TTS_VOICE_NAME)
    ...
    def test_detects_font_without_chinese_glyphs(self):
        ...
        latin_font = fonts_dir / "BeVietnamPro-Bold.ttf"
        chinese_font = fonts_dir / "MicrosoftYaHeiBold.ttc"
        if not latin_font.is_file() or not chinese_font.is_file():
            self.skipTest("resource/fonts fixtures are not present")
